...
// --------------------------------------------------------------------------------------------------------
        // Hides the base.ForeColor property from the Properties tab in IDE.
        // Properties for using are TitleForeColor and ButtonsForeColor
        [Browsable(false)]
        ...
        // Changing control's ForeColor property do not change color of the groupBox's title.
        // So the only way to change the title's color is to assign to the groupBox.ForeColor property.
        // Assignment to TitleForeColor property can change the ForeColor of buttons too.
        [Category("Appearance")]
        [Description("Defines the font color of the control's title. Set the groupBox.ForeColor property.")]
        ...
        public virtual Color TitleForeColor
        {
            ...
            {
                titleColor = value;
                theGrpBox.ForeColor = value;
            }
        }

        // The button's color can differ from the color of the title.
        [Category("Appearance")]
        [Description("Defines the font color of the control's radiobuttons.")]
        [DefaultValue(typeof(Color), "Black")]
        public virtual Color ButtonsForeColor
        {
            get
            {
                return buttonsColor;
            ...
            set
            {
                if (value != buttonsColor)
                {
                    buttonsColor = value;
                    this.buttons.ForEach(b => b.ForeColor = this.buttonsColor);
                }
            }
        }

        // Restores effect of the control's ForeColor property.
        public void ResetForeColors()
        {
            this.ButtonsForeColor = default(Color);
            this.TitleForeColor = default(Color);
        }
// ---------------------------------------------------------------------------------------------------
        ...
        // Calculates and sets the control's minimum size to be all radioButtons visible.
        public void SetCorrectMinimumSize()
        {
            float fs = this.Font.Size;
            int width = buttons.Max(but => but.Width);
            Size maxSize = new Size((width + 4) * columnCount + (int)(fs * 1.4 + 0.5),
                (buttons[0].Height + 5) * rowCount + (int)(fs * 2.8 + 0.5));
